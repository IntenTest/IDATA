...
UTF-8 under `%USERPROFILE%\.idata\server-command-runtime\logs\<run-id>` and copied
into the run details when each case finishes. During execution, each status poll
reads a bounded tail of the running case log into the response without rewriting
run state. The web page polls every two seconds; log read failures preserve the
last persisted output and do not interrupt status polling. Authorized users can download each
