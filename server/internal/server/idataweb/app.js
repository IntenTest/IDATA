...
const App = {
  setup() {
    ...
    function setRouteForView(view, runId = "") {
      const route =
        view === "Test Run Details"
          ...
          : viewRoutes[view] || "";
      const nextParams = new URLSearchParams(route);
      for (const key of ["client", "mode"]) {
    ...
    async function loadSettings() {
      ...
      try {
        const response = await window.idataFetch("/api/settings", { cache: "no-store" });
    ...
    async function saveSettings() {
      ...
      try {
        const response = await window.idataFetch("/api/settings", {
          method: "PUT",
    ...
    async function loadModelConfig() {
      ...
      try {
        const response = await window.idataFetch("/api/model-config", { cache: "no-store" });
        const result = await response.json();
        if (!response.ok) {
    ...
    async function saveModelConfig() {
      ...
      modelConfigError.value = "";
      try {
        const response = await window.idataFetch("/api/model-config", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
    ...
    async function loadTestCases() {
      ...
      try {
        const response = await window.idataFetch("/api/test-cases", { cache: "no-store" });
        const result = await response.json();
        if (!response.ok) {
          throw new Error(
            result.error ||
    ...
    async function updateTestCaseLibrary() {
      ...
      try {
        let response = await window.idataFetch("/api/test-cases/update", { method: "POST" });
        for (;;) {
          const result = await response.json();
          ...
          response = await window.idataFetch("/api/test-cases/update", { cache: "no-store" });
        }
    ...
    async function closeTestRun() {
      ...
      try {
        ...
        const response = await window.idataFetch(`/api/test-runs/${runId}/close`, {
          method: "POST",
        });
        const result = await response.json();
        if (!response.ok) {
    ...
    async function startTestRun() {
      ...
      try {
        const response = await window.idataFetch("/api/test-runs", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
