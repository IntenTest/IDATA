...
func idataWorkerCommand(goos, operation, method string) string {
	...
	if goos == "windows" {
		...
		script := fmt.Sprintf(`$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; $root=Join-Path $env:LOCALAPPDATA 'IDATA\server-command-runtime'; [IO.Directory]::CreateDirectory($root) | Out-Null; $token=[guid]::NewGuid().ToString('N'); $worker=Join-Path $root ($token+'.ps1'); $request=Join-Path $root ($token+'.json'); $utf8=New-Object System.Text.UTF8Encoding($false); $scriptUtf8=New-Object System.Text.UTF8Encoding($true); [Console]::InputEncoding=$utf8; [Console]::OutputEncoding=$utf8; try{$wire=[Console]::In.ReadToEnd(); $split=$wire.IndexOf([char]10); if($split -lt 0){throw 'Invalid Server input'}; $payload=$wire.Substring(0,$split).TrimEnd([char]13); [IO.File]::WriteAllText($worker,$wire.Substring($split+1),$scriptUtf8); if($payload){[IO.File]::WriteAllBytes($request,[Convert]::FromBase64String($payload))}else{[IO.File]::WriteAllText($request,'',$utf8)}; $workerOutput=(& $worker -RequestPath $request -OperationB64 '%s' -MethodB64 '%s' 2>&1 | Out-String); if($workerOutput.Contains('__IDATA_SERVER_RESPONSE__')){[Console]::Out.Write($workerOutput); exit 0}; $detail=$workerOutput.Trim(); if($detail.Length -gt 2000){$detail=$detail.Substring($detail.Length-2000)}; [Console]::Error.WriteLine(('Server PowerShell worker returned no response. '+$detail)); exit 1}catch{[Console]::Error.WriteLine(('Server PowerShell worker failed. '+$_.Exception.Message)); exit 1}finally{Remove-Item -LiteralPath $request,$worker -Force -ErrorAction SilentlyContinue}`, encodedOperation, encodedMethod)
