...
func TestIDATAWorkspaceIsTheOnlyEmbeddedWebPage(t *testing.T) {
	...
	for _, label := range []string{"Open IDATA Client", "启动 IDATA 客户端", "Install IDATA Client", "安装IDATA Client客户端", "https://openx.huawei.com/IDATA/download", "Current web access IP", "Matched Client connection IP", "<el-dialog", ":close-on-click-modal=\"false\"", ":close-on-press-escape=\"false\"", ":show-close=\"false\""} {
	...
	if !strings.Contains(response.Body.String(), "remote.js?v=20260916-client-download") {
