# IDATA Linux 服务器重新部署（v0.2.51）

本文用于在 Ubuntu x86-64 服务器上首次安装或升级 IDATA Server。
重新部署会保留现有的监听地址、Token 和已批准的设备凭据。

本次升级只需更新 Ubuntu Server（0.2.51）。网页和 Windows/Python 执行脚本已嵌入 Server。
...
本次新增：设置页显示当前 Server 版本，版本号直接来自 Server 健康检查接口，
并与已部署的 GitHub Release 标签一致。从 v0.2.50 升级无需迁移数据。
...
升级前结束正在执行的任务。如果需要保留历史任务和设置，先备份并将旧的
%USERPROFILE%\.idata 整个目录复制到 D:\.idata（保留旧目录，不要删除）。
如果 D:\.idata 已有数据，请先备份双方并核对，避免覆盖新任务。
复制后将 server-command-runtime/settings.json 中指向旧目录的 testCaseLibraryPath
改为 D:/.idata/newest_testcases，
或在网页设置中选择新目录。历史记录中的报告/日志绝对路径仍引用原文件；保留旧文件可继续查看。
如果不迁移，页面将从新的空目录开始，重新更新测试用例库即可；旧数据不会被删除。
刷新网页后验证任务状态、测试用例库、报告打开和 Client 连接。

## 1. 需要下载的文件

推荐只下载以下两个文件：

1. [`IDATA-ubuntu-v0.2.51.tar.gz`](https://github.com/IntenTest/IDATA/releases/download/v0.2.51/IDATA-ubuntu-v0.2.51.tar.gz)
   —— Ubuntu 完整部署包，内含 Linux Server、部署脚本、本文档和包内校验文件。
2. [`SHA256SUMS`](https://github.com/IntenTest/IDATA/releases/download/v0.2.51/SHA256SUMS)
   ——用于校验下载的 `.tar.gz` 是否完整。

Linux 服务器不需要下载 `IDATA-Client.exe`；该文件只用于
Windows 执行电脑。Windows 用户应从同一 Release 下载
[`IDATA-Client.exe`](https://github.com/IntenTest/IDATA/releases/download/v0.2.51/IDATA-Client.exe)。

如果 Ubuntu 服务器可以访问 GitHub，直接执行：

```bash
mkdir -p "$HOME/idata-release-v0.2.51"
cd "$HOME/idata-release-v0.2.51"
curl --fail --location --remote-name \
  https://github.com/IntenTest/IDATA/releases/download/v0.2.51/IDATA-ubuntu-v0.2.51.tar.gz
curl --fail --location --remote-name \
  https://github.com/IntenTest/IDATA/releases/download/v0.2.51/SHA256SUMS
```
...
```bash
grep ' IDATA-ubuntu-v0.2.51.tar.gz$' SHA256SUMS | sha256sum --check -
tar -xzf IDATA-ubuntu-v0.2.51.tar.gz
cd IDATA-ubuntu-v0.2.51
