# IDATA Linux 服务器重新部署（v0.2.50）

本文用于在 Ubuntu x86-64 服务器上首次安装或升级 IDATA Server。
重新部署会保留现有的监听地址、Token 和已批准的设备凭据。

本次升级只需更新 Ubuntu Server（0.2.50）。网页和 Windows/Python 执行脚本已嵌入 Server。
...
本次调整：管理测试任务表格操作列的删除按钮文案简化为“删除”。
从 v0.2.49 升级无需迁移数据；升级完成后刷新浏览器即可。
...
1. [`IDATA-ubuntu-v0.2.50.tar.gz`](https://github.com/IntenTest/IDATA/releases/download/v0.2.50/IDATA-ubuntu-v0.2.50.tar.gz)
   —— Ubuntu 完整部署包，内含 Linux Server、部署脚本、本文档和包内校验文件。
2. [`SHA256SUMS`](https://github.com/IntenTest/IDATA/releases/download/v0.2.50/SHA256SUMS)
   ——用于校验下载的 `.tar.gz` 是否完整。

Linux 服务器不需要下载 `IDATA-Client.exe`；该文件只用于
Windows 执行电脑。Windows 用户应从同一 Release 下载
[`IDATA-Client.exe`](https://github.com/IntenTest/IDATA/releases/download/v0.2.50/IDATA-Client.exe)。

如果 Ubuntu 服务器可以访问 GitHub，直接执行：

```bash
mkdir -p "$HOME/idata-release-v0.2.50"
cd "$HOME/idata-release-v0.2.50"
curl --fail --location --remote-name \
  https://github.com/IntenTest/IDATA/releases/download/v0.2.50/IDATA-ubuntu-v0.2.50.tar.gz
curl --fail --location --remote-name \
  https://github.com/IntenTest/IDATA/releases/download/v0.2.50/SHA256SUMS
```
...
```bash
grep ' IDATA-ubuntu-v0.2.50.tar.gz$' SHA256SUMS | sha256sum --check -
tar -xzf IDATA-ubuntu-v0.2.50.tar.gz
cd IDATA-ubuntu-v0.2.50
